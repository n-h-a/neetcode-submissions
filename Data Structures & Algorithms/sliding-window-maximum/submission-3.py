class Solution:
    def maxSlidingWindow(self, nums: List[int], k: int) -> List[int]:
        ...
        while r < len(nums):
          ...
          # Ensure first sliding window exists.
          if (r + 1) >= k:
...
# IDEA: Go through list,
#   Add candidates to list.
#   If you find a candidate greater than some previous ones, remove the
#     smaller ones.
#   This maintains a list that is decreasing.
# Use a deque.
#   Property: Descending, so
#     max will always be leftmost.
#   If we see a value > prev values in window,
#     pop prev values from window until you reach one >=.
