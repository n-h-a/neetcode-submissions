...
# Use a ton of nodes to save values:
#   Use a dummy node b/c we'll be modifying the head.
#   Save the kth node, the node before a group, and the node after the group.
# Perform reverse a singly linked list logic but ensure that
#   we update the old beginning to point to the next group.
# TC: O(N)
# SC: O(1)
