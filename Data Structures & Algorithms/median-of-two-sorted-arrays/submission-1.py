class Solution:
    def findMedianSortedArrays(self, nums1: List[int], nums2: List[int]) -> float:
        ...
        def solve(a, b):
            # Small enough: just brute force this part
            if len(a) + len(b) <= 4:
                merged = sorted(a + b)
                n = len(merged)
                if n % 2:
            ...
            # Always discard the same number from opposite sides
            k = min(len(a), len(b)) // 2

            if ma < mb:
                # Drop lower part of a, upper part of b
                return solve(a[k:], b[:len(b) - k])
            else:
                # Drop upper part of a, lower part of b
                return solve(a[:len(a) - k], b[k:])
